...
-(void)pushTimeButtonPressed {
    //本地化一下子
    UIActionSheet *asheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"请选择每日助理提醒您学习的时间", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"取消", nil) destructiveButtonTitle:nil otherButtonTitles:NSLocalizedString(@"确认", nil), nil];
...
//    [cell setSelectionStyle:UITableViewCellSelectionStyleGray];
    ...
    if ([[specifier type] isEqualToString:kIASKPSToggleSwitchTouchSpecifier]) {
        if (!cell) {
            cell = (IASKPSToggleSwitchTouchSpecifierViewCell*) [[[NSBundle mainBundle] loadNibNamed:@"IASKPSToggleSwitchTouchSpecifierViewCell" owner:self options:nil] objectAtIndex:0];
